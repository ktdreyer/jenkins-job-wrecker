...
class Job(object):
    ...
    # Walk an XML ElementTree ("root"), and print YAML to STDOUT.
    def print_job(self, root):
        # Top-level "job" data
        job = {}
        build = [{'job': job}]
    ...
    def handle_properties(self, top):
        ...
        for child in top:
            ...
            if child.tag == 'com.coravy.hudson.plugins.github.GithubProjectProperty':
                github = self.handle_github_project_property(child)
                properties.append(github)
            # Parameters
            elif child.tag == 'hudson.model.ParametersDefinitionProperty':
                parametersdefs = self.handle_parameters_property(child)
    ...
    def handle_github_project_property(self, top):
        github = []
        for child in top:
    ...
    def handle_parameters_property(self, top):
        parameters = []
    ...
    def handle_publishers(self, top):
        publishers = []
        for child in top:
    ...
    def handle_buildwrappers(self, top):
        wrappers = []
        for child in top:
    ...
    def handle_executionstrategy(self, top):
        strategy = []
        for child in top:
